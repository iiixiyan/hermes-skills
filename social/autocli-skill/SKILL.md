...
- autocli Chrome extension installed (for browser commands) — download from https://github.com/nashsu/AutoCLI/releases/latest

### ⚠️ 安装后第一件事：运行 `autocli doctor`
...
| 项目 | 含义 | 修复 |
|:----|:-----|:-----|
| ✓ Chrome/Chromium | 浏览器可执行文件存在 | ✅ 正常 |
| ✓ Daemon running | autocli 后台进程运行 | 若未运行，`autocli` 会自动启动 |
| ✗ Chrome extension connected | Chrome 扩展未连接 | 用户需手动安装扩展并连接 |

**浏览器模式（Browser Mode）的所有命令**（Twitter/B站/小红书/微博/知乎等）需要 **三要素齐全**（Chrome运行中 + 扩展已连接 + 已登录目标网站），缺一则无法工作。

**公共模式（Public Mode）**（HackerNews/Arxiv/BBC/V2EX等）无需浏览器，安装即用。
