...
### curl 直接调用 MCP API（无需 mcporter）

当 mcporter 或 Hermes 原生 MCP 客户端不可用时，可直接通过 curl 调用：
...
完整调用模板见 `references/curl-mcp-archive-workflow.md`。
...
- **禁止用 smartcanvas.edit 写入内容**：`smartcanvas.edit` (INSERT_BEFORE等) 写入后可能返回空结果，导致文档内容实际为空。正确做法：`create_smartcanvas_by_mdx` 创建时直接传入 `mdx` 内容，然后 `manage.move_file` 移动到目标文件夹。详见 `references/curl-mcp-archive-workflow.md`
