...
```
1. 确认优化范围：
   - 全部skills → 扫描 skills 目录下的 SKILL.md（`~/.claude/skills/` 或 `~/.hermes/skills/`）
   - 指定skills → 用户指定列表
2. 创建 git 分支或文件备份：auto-optimize/YYYYMMDD-HHMM（git 仓库）或 `cp SKILL.md SKILL.md.bak.YYYYMMDD-HHMM`（非 git 目录）
...
| 运行在 Hermes（非Claude Code） | `~/.hermes/skills/` 不是 git repo | 用文件备份代替 git revert（详见 references/hermes-adaptation.md） |
