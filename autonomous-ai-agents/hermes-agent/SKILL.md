...
- **Weixin/WeChat iLink Bot setup via QR code (non-interactive environments)**:
  - The setup wizard (`hermes gateway setup` → Weixin) is interactive. In headless/Feishu sessions, perform the flow programmatically:
    1. Call `_api_get` on `/ilink/bot/get_bot_qrcode?bot_type=3` with proper headers (`iLink-App-Id: bot`, `iLink-App-ClientVersion`)
    2. The response contains `qrcode_img_content` (a `liteapp.weixin.qq.com` URL) — display as an image or link for the user to scan
    3. Poll `/ilink/bot/get_qrcode_status?qrcode=<value>` every 2-3s for status changes: `wait` → `scaned` → `confirmed`
    4. On `confirmed`, save `ilink_bot_id` as `WEIXIN_ACCOUNT_ID` and `bot_token` as `WEIXIN_TOKEN` into `~/.hermes/.env`
    5. Set `WEIXIN_DM_POLICY=pairing` and add `platforms.weixin.enabled: true` to `config.yaml`
    6. Verify: restart gateway (`hermes gateway restart`), then approve the user via `hermes pairing approve weixin <CODE>`
